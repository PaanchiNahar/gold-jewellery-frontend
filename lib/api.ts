...
export async function fetchStock(filters: any) {
  try {
    ...
    return mockData.filter((item) => {
      ...
      if (filters.merchant && item.merchantName !== filters.merchant) match = false
      if (filters.purity && item.purity !== filters.purity) match = false
      if (filters.search) {
        const searchLower = filters.search.toLowerCase()
        const matchesSearch =
          item.ornamentId.toLowerCase().includes(searchLower) || item.merchant.toLowerCase().includes(searchLower)
...
export async function fetchSalesHistory(filters: any) {
  try {
    ...
    return mockData.filter((item) => {
      ...
      if (filters.paymentMethod && item.paymentMethod !== filters.paymentMethod) match = false
      if (filters.search) {
        const searchLower = filters.search.toLowerCase()
        const matchesSearch =
          item.billId.toLowerCase().includes(searchLower) || item.clientName.toLowerCase().includes(searchLower)
        if (!matchesSearch) match = false
      }
